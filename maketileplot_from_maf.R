...
option_list = list(
	make_option(c("-a", "--fileA"), type = "character", default = NULL, 
              help = "dataset file name", metavar = "character"),
	make_option(c("-b", "--fileB"), type = "character", default = NULL, 
              help = "dataset file name", metavar = "character"),
	make_option(c("-c", "--cutoff"), type = "integer", default = NULL, 
	...
	make_option(c("-f", "--filter_col"), type = "character", default = NULL, 
              help = "Column name and value to filter on (colname, value) [default = %default]", metavar = "character"),
	make_option(c("-m", "--sortbymutations"), action = "store_true", default = FALSE, 
              help = "Sort genes by number of mutations"),
	make_option(c("-n", "--sortbysamplemut"), action = "store_true", default = FALSE, 
              help = "Sort samples by number of mutations"),
	make_option(c("-l", "--sortbygenelist"), action = "store_true", default = FALSE, 
              help = "Sort genes by order given in genelist file"),
	make_option(c("-e", "--sortbysamplelist"), action = "store_true", default = FALSE, 
              help = "Sort sample by order given in sample file"),
	make_option(c("--sortbyfrequency"), action = "store_true", default = FALSE, 
	...
	make_option(c("-p", "--prefix"), type = "character", default = "gene_tileplot", 
	...
	make_option(c("-A", "--relsizeA"), type = "integer", default = 1, 
	...
	make_option(c("-B", "--relsizeB"), type = "integer", default = 1, 
	...
	make_option(c("--vline"), type = "character", default = NULL, 
	...
	make_option(c("--hline"), type = "character", default = NULL, 
	...
	make_option(c("--sizex"), type = "character", default = 8, 
              help = "Font size for x-axis text [default = %default]"),
	make_option(c("--sizey"), type = "character", default = 8, 
              help = "Font size for x-axis text [default = %default]"),
	make_option(c("--frequency"), type = "character", default = NULL,
              help = "Tab-separated file containing gene and frequency to plot in a heatmap [default = %default]"),
	make_option(c("--sortbysidebar"), action = "store_true", default = FALSE, 
...
shorten_consequence <- function(df) {
	...
	data <- data.frame(lapply(data, function(x)  { gsub("missense_variant", "Missense", x)}))
	data <- data.frame(lapply(data, function(x)  { gsub("stop_gained", "Nonsense", x)}))
	data <- data.frame(lapply(data, function(x)  { gsub("splice_acceptor_variant", "Splice site", x)}))
	data <- data.frame(lapply(data, function(x)  { gsub("splice_donor_variant", "Splice site", x)}))
	data <- data.frame(lapply(data, function(x)  { gsub("frameshift_variant", "Frameshift", x)}))
	data <- data.frame(lapply(data, function(x)  { gsub("inframe_deletion", "Inframe indel", x)}))
	data <- data.frame(lapply(data, function(x)  { gsub("inframe_insertion", "Inframe indel", x)}))
	data <- data.frame(lapply(data, function(x)  { gsub("synonymous_variant", "Synonymous", x)}))
	data <- data.frame(lapply(data, function(x)  { gsub("stop_retained_variant", "Synonymous", x)}))
	data <- data.frame(lapply(data, function(x)  { gsub("start_retained_variant", "Synonymous", x)}))
	data <- data.frame(lapply(data, function(x)  { gsub("coding_sequence_variant", "Other", x)}))
	data <- data.frame(lapply(data, function(x)  { gsub("protein_altering_variant", "Other", x)}))
	data <- data.frame(lapply(data, function(x)  { gsub("stop_lost", "Start/stop codon lost", x)}))
	data <- data.frame(lapply(data, function(x)  { gsub("start_lost", "Start/stop codon lost", x)}))
	data <- data.frame(lapply(data, function(x)  { gsub("incomplete_terminal_codon_variant", "Other", x)}))
	data <- data.frame(lapply(data, function(x)  { gsub("tanscript_ablation", "Other", x)}))
...
order_by_freq2 <- function(data, gene_col, sample_col, cons_col) {
	...
	gene_counts <- data_tmp %>% select(Gene_tmp, Sample_tmp) %>%
		distinct() %>% count(Gene_tmp, sort = T)
...
filter_by_minmutations <- function(data, cutoff, gene_col, sample_col) {
	...
	gene_counts <- data %>% select(!!as.name(gene_col), !!as.name(sample_col)) %>%
		distinct() %>% add_count(!!as.name(gene_col)) %>%
		filter(n >= cutoff)
...
sort_genes_by_mutations <- function(data, gene_col, sample_col) {
	...
	gene_counts <- data %>% select(!!as.name(gene_col), !!as.name(sample_col)) %>%
		distinct() %>% count(!!as.name(gene_col), sort = T)
...
sort_samples_by_mutations <- function(data, gene_col, sample_col) {
	...
	sample_counts <- data %>% select(!!as.name(gene_col), !!as.name(sample_col)) %>%
		distinct() %>% count(!!as.name(sample_col), sort = T, .drop = F)
...
group_samples_by_gene <- function(data, gene_col, sample_col, group) {
	...
	for (gene in group_list) {
		sample_match <- data %>% filter(!!as.name(gene_col) == gene) %>%
			select(!!as.name(sample_col)) %>% droplevels()
...
make_mutation_plot <- function(data, legendpos = "top", nrow = 2, ncol = 5, vlines = NULL, hlines = NULL, gene_col = gene_col, sample_col = sample_col, cons_col = cons_col) {
	...
	data <- data %>% select(!!as.name(gene_col), !!as.name(cons_col), !!as.name(sample_col)) %>% 
...
#	# now make the Effect levels the same, for plotting
	...
	singles <- data %>% add_count(Gene, Sample) %>% filter(n == 1 | n > 2) %>% 
				mutate(Effect = replace(as.character(Effect), n > 2, "More than 2 mutations")) %>%
				distinct(Gene, Sample, .keep_all = TRUE) %>% select(-n)
	doubles <- data %>% add_count(Gene, Sample) %>% filter(n == 2) %>% select(-n)
...
#	data$Effect <- factor(data$Effect, levels = legendtext2$order)
	...
	doubles <- doubles %>% group_by(Gene, Sample) %>% mutate(id = row_number())
...
#	str(data.all.tmp)
	missing <- melt(dcast(data.all.tmp, Gene ~ Sample.index, value.var = "Effect", fun.aggregate = length, drop = FALSE), id.var = "Gene") %>%
		filter(value == 0) %>% select(-value) %>% rename(Sample.index = variable) %>% mutate(Sample.index = as.numeric(Sample.index))
	missing["Effect"] <- NA
...
make_frequency_plot <- function(data, gene_col, perc_col, group) {
	...
	plot <- ggplot(data, aes(x = !!as.name(group), y = !!as.name(gene_col), fill = !!as.name(perc_col))) +
		geom_tile(aes(width = 0.5, height = 1), colour = "white", linewidth = 0.2, alpha = 0.75) +
			...
			scale_fill_viridis_c(direction = -1, na.value="white", limits = c(0, max_scale), breaks = scale_breaks,
...
#	data <- data %>% group_by(as.character(Hugo_Symbol))
	...
	if (! is.null(opt$frequency)) {
		...
		if (! isTRUE(opt$sortbysidebar)) {
			data.freq <- data %>% select(Tumor_Sample_Barcode, Hugo_Symbol) %>% left_join(x = ., y = data.freq, by = "Hugo_Symbol") 
