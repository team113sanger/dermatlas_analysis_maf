...
top_sites <- function(df) {
	top <- df %>% select(Gene, Hugo_Symbol, Chromosome, POS_VCF, Reference_Allele, Tumor_Seq_Allele2) %>%
...
process_variants <- function(df) {
	df <- df %>% filter(str_detect(Keep, "keep")) %>%
...
make_af_vs_d_barplots <- function(df, ncol) {
	data <- df %>% select(Tumor_Sample_Barcode, Mutation, Direction) %>%
		group_by(Tumor_Sample_Barcode, Mutation, Direction) %>% mutate(Count = n()) %>%
		distinct() %>%
		data.frame()
